...
def sync_jaaba_with_ros(FMF_DIR):
    ...
    number_of_bouts, bout_duration, first_TS, last_TS = utilities.detect_stim_bouts(jaaba_data, 'Laser2_state')
    ...
    number_of_bouts, bout_duration, first_TS, last_TS = utilities.detect_stim_bouts(jaaba_data, 'Laser2_state')  #HACK DANNO
...
def gather_data(filelist):
    ...
    for x in filelist:
        ...
        if WEI < WEI_THRESHOLD:
            ...
            continue
        fx['group'] = str(number_of_bouts) + 'x' + str(bout_duration) + 's' 
        print str(number_of_bouts) + 'x' + str(bout_duration) + 's' 
        fx['FlyID'] = FLY_ID
