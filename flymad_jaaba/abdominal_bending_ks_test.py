...
def get_ks_stats(parent_dir, period):
    group1 = []
    group2 = []
    itercount = 0
    for directory in glob.glob(DATADIR + '*' + HANDLE + '*' + '*zoom*'):
        ...
        if period == 'post':
            TEMP_full = TRACKING_DATA[TRACKING_DATA['synced_time'] > 0]['Length'].values
            if _pop == 'A':
                group1.extend(TEMP_full)
            elif _pop == 'B':
                group2.extend(TEMP_full)
        elif period == 'pre':
            TEMP_full = TRACKING_DATA[TRACKING_DATA['synced_time'] < 0]['Length'].values
            if _pop == 'A':
                group1.extend(TEMP_full)
    ...
    return st.ks_2samp(group1, group2)
...
pre_D, pre_P = get_ks_stats(DATADIR, 'pre')
post_D, post_P = get_ks_stats(DATADIR, 'post')


print 'PRE-STIMULUS: ', pre_P, '  POST-STIMULUS: ', post_P
