...
def make_panel(flyInstance, row, column, _frame_number):
        ...
        fig.add_axes(ax1)
        frame,timestamp = flyInstance.get_frame((_frame_number + flyInstance._Tzero + 1800))     #1800 starts @120sec
        ...
        fig.add_axes(ax2)
        flyInstance.plot_extending_line(timestamp, ax2, 'maxWingAngle', 'r', 'Wing Angle (rad)', _left_bound, _right_bound, 'no_titles')
        plt.setp(ax2.get_xticklabels(), visible=False)
        
        ax3 = plt.subplot2grid((60,6), (row+14,2), colspan=4, rowspan=10)
        
        fig.add_axes(ax3)
        flyInstance.plot_extending_line(timestamp, ax3, 'dtarget', 'b', 'Distance (px)', _left_bound, _right_bound, 'w_titles')        
