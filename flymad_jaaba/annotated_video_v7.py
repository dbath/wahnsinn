...
def plot_window_of_data(timestamp, data, windowsize, ax, measurement, colour, title):
...
def make_png_set(fmf_dir, save_dir):
    ...
    for frame_number in range(3500):
        ...
        fig = plt.figure(figsize=(image_width/100, image_height*1.2/100), dpi=200.399 )
        ...
        ax2 = plt.subplot2grid((12,10), (10,0), colspan=10)
        fig.add_axes(ax2)
        plot_window_of_data(timestamp, jaaba_data, 20, ax2, 'dtarget', 'b', 'Distance')
        plt.setp(ax2.get_xticklabels(), visible=False)
        
        ax3 = plt.subplot2grid((12,10), (11,0), colspan=10)
        fig.add_axes(ax3)
        plot_window_of_data(timestamp, jaaba_data, 20, ax3, 'maxWingAngle', 'r', 'Wing Angle')        
        
        plt.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=1.0, wspace=0.0, hspace=0.05)
