...
FLUOR_FILE = '/tier2/dickson/bathd/160829_josh_gcamp_data/160908_trial_dataset/1.txt'
TEMP_FILE = '/tier2/dickson/bathd/160829_josh_gcamp_data/160908_trial_dataset/temp1.txt'
read_column = 'Intensity Region 1'
background_column = 'Intensity Region 2'
BASELINE_DURATION = 10 #seconds
SAMPLING_RATE = '500ms'

jdict = pd.read_csv('/groups/dickson/dicksonlab/forDan/lillvis_p1-mAL-trp/Directories_161019.csv')
...
#jdict = pd.read_pickle('/groups/dickson/dicksonlab/forDan/lillvis_p1-mAL-trp/jdict.pickle')
...
for expNum in range(len(jdict)):
    try:
        ...
        plt.savefig('/groups/dickson/dicksonlab/forDan/lillvis_p1-mAL-trp/'+filename)
        plt.close('all')
...
data = pd.read_table(FLUOR_FILE)
data = data[:-1]
data.index = pd.to_datetime(data['Time [s]'], unit='s')
data['df'] = data[read_column] - data[background_column]
baseline = data[data.index <= pd.to_datetime(BASELINE_DURATION*1e9)]['df'].mean()
data['dff'] = (data['df'] - baseline) / baseline #data['df']


temp = pd.read_table(TEMP_FILE)
temp.index = pd.to_datetime(temp['Time'], unit='s')

foo = data.merge(temp, how='outer', left_index=True, right_index=True).resample(SAMPLING_RATE)
