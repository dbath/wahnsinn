...
def plot_from_csv(mean, sem):
    ...
    for j in mean.columns[:-1]:
        ...
        ax.set_ylim(mean[np.isfinite(means)].min().min() ,means[np.isfinite(means)].max().max())
        #ax.set_ylabel(INDEX_NAME[params.index(i)] + ' ' + u"\u00B1" + ' SEM')   # +/- sign is u"\u00B1"
        ax.set_ylabel('Following Index ' + u"\u00B1" + ' SEM')
    ...
    plt.show()
    fig.savefig('/groups/dickson/home/bathd/Dropbox/THESIS/X011_pIP10_adt2_co-activation/following_plot.svg', bbox_inches='tight')


colourlist = ['g', 'k', 'purple','k','g','r','c','m','y']

    
means = pd.read_csv('/groups/dickson/home/bathd/Dropbox/THESIS/X011_pIP10_adt2_co-activation/followingbytemperature_groupedbygenotype_means.csv',sep=',', index_col=0, header=0)
sems = pd.read_csv('/groups/dickson/home/bathd/Dropbox/THESIS/X011_pIP10_adt2_co-activation/followingbytemperature_groupedbygenotype_SEM.csv',sep=',', index_col=0, header=0)
